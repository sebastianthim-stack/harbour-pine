...
2. **Set the first sheet tab to "Published to the web".**
   `File → Share → Publish to the web` → select the whole sheet → **Publish**.
   This is what makes the JSONP endpoint work without authentication.

3. **Import the seed data.** In the first tab, `File → Import → Upload`, then
   choose `data/harbour_pine_inventory.csv` → *Replace current sheet*.
